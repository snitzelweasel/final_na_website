document.addEventListener('DOMContentLoaded', function() {
    ...
    const hobbyModalTitle = document.getElementById('hobby-modal-title');
    const hobbyModalImage = document.getElementById('hobby-modal-image');
    const hobbyModalDescription = document.getElementById('hobby-modal-description');
    const closeHobbyModal = document.querySelector('.close-hobby-modal');
    ...
    const hobbies = {
        fitness: {
            ...
            description: "I'm passionate about staying fit and healthy through regular gym workouts. I enjoy strength training, cardio exercises, and maintaining a consistent fitness routine that keeps me energized and motivated throughout the day."
        },
        movies: {
            title: 'Watching Movies',
            image: 'images/movies.jpg',
            description: "I love exploring different genres of films, from thrilling action movies to thought-provoking dramas and captivating sci-fi adventures. There's nothing quite like getting immersed in a great story and experiencing the artistry of cinematography and storytelling."
        }
    };

    // Open hobby modal function (global for onclick)
    window.openHobbyModal = function(hobbyKey) {
        const hobby = hobbies[hobbyKey];
        if (hobby) {
            hobbyModalTitle.textContent = hobby.title;
            hobbyModalImage.src = hobby.image;
            hobbyModalImage.alt = hobby.title;
            hobbyModalDescription.textContent = hobby.description;
    ...
    // Close hobby modal
    function closeHobbyModalFunction() {
        hobbyModal.style.display = 'none';
        document.body.style.overflow = 'auto';
    }

    // Event listeners for hobby modal
    closeHobbyModal.addEventListener('click', closeHobbyModalFunction);

    // Close modal when clicking outside
    hobbyModal.addEventListener('click', function(event) {
        if (event.target === hobbyModal) {
            closeHobbyModalFunction();
        }
    });

    // Close hobby modal with Escape key
    document.addEventListener('keydown', function(event) {
        if (event.key === 'Escape' && hobbyModal.style.display === 'block') {
            closeHobbyModalFunction();
        }
    });
});